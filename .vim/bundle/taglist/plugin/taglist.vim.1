...
if !exists('loaded_taglist')
    ...
    " OLA ++
    let s:tlist_space_title = ' '
    let s:tlist_space_tag = '  '
    " OLA --
...
function! s:Tlist_Window_Init()
    ...
    exec "syntax match TagListTitle /^" . s:tlist_space_title . "\\S.*$/"
    " OLA **
...
function! s:Tlist_Window_Refresh_File(filename, ftype)
    ...
    while i <= ttype_cnt
        ...
        if ttype_txt != ''
            " OLA **
            let txt = s:tlist_space_title . s:tlist_{a:ftype}_{i}_fullname
            " OLA **
            if g:Tlist_Compact_Format == 0
...
function! s:Tlist_Parse_Tagline(tag_line)
    ...
    if g:Tlist_Display_Prototype
        let ttxt = s:tlist_space_tag . s:Tlist_Get_Tag_Prototype(s:fidx, s:tidx)
    else
        let ttxt = s:tlist_space_tag . tag_name
    " OLA **
...
function! s:Tlist_Process_File(filename, ftype)
    ...
    if v:version > 601
        ...
        while cmd_output != ''
            ...
            " Extract the tag scope/prototype
            if g:Tlist_Display_Prototype
                let ttxt = s:tlist_space_tag . s:Tlist_Get_Tag_Prototype(fidx, tidx)
            else
                let ttxt = s:tlist_space_tag . tag_name
            " OLA **
                " Add the tag scope, if it is available and is configured. Tag
