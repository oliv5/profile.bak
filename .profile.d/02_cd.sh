...
function cdb()  { pop g_backward_stack DIR 2>/dev/null && push g_forward_stack "$DIR" && pkeepq g_forward_stack $g_forward_stack_maxsize && PUSH=false cd "$DIR"; }
function cdf()  { pop g_forward_stack DIR 2>/dev/null && push g_backward_stack "$DIR" && pkeepq g_backward_stack $g_backward_stack_maxsize && PUSH=false cd "$DIR"; }
