...
function! s:Project(filename) " <<<
    ...
    " OLA ++
    if !exists('g:proj_window_pos')             " Default window position: left
      let g:proj_window_pos='H'
    endif
    " OLA --
    ...
    if !exists("g:proj_running") || (bufwinnr(g:proj_running) == -1) " Open the Project Window
        ...
        if match(g:proj_flags, '\CF') == -1      " We're floating
            " OLA ++
            exec 'silent! wincmd '.g:proj_window_pos
            echo 'silent! wincmd '.g:proj_window_pos
            "silent! wincmd H
            " OLA --
        ...
        " OLA ++
        exec 'silent! 99wincmd' tolower(g:proj_window_pos)
        "silent! 99wincmd h
        " OLA --
    ...
    function! s:OpenEntry2(line, infoline, fname, editcmd)
        ...
        " OLA ++
        if exists("*g:wndmgr_JumpEditWnd")
...
" OLA ++
if exists(':ProjectClose') != 2
  command -nargs=0 ProjectClose call <SID>DoProjectClose()
endif
function! s:DoProjectClose()
  if exists('g:proj_running') && bufwinnr(g:proj_running) != -1
      let g:proj_mywindow = winnr()
      Project
      hide
